...
A sala de estudo ganhou um **modo cinema** (TASK-010), a pedido do proprietário e apenas acima de 1240px. A primeira versão só invertia as colunas dentro da página e ficou pior que o arranjo normal num notebook: a sala tem altura de janela, e entre cabeçalho, aviso e trilha de etapas sobravam ~240px para o vídeo. Agora o modo é uma superfície — barra lateral, aviso e trilha saem, o cabeçalho vira faixa fina com título e fechar, e a janela inteira é vídeo mais transcrição de 320–440px. Em 1360×630 o vídeo passa de 380×220 para 852×485; em 1920×1080, para 1398×792. O palco não tem fundo próprio, então não há moldura preta em volta. Sai pelo botão, por `Esc` ou por `C`; a preferência fica em `localStorage` (`fq-cinema`). Celular e tablet não mudaram.
...
- **Drift de tempo varia muito entre vídeos** (impacto médio): 1,118× no vídeo de 837 s em 16/09, acima de 1,5× no vídeo de 636 s em 17/09. `MAX_TIME_DRIFT` subiu de 1,5 para 10 — o teto agora discrimina erro de unidade (1000×), não desvio de estimativa —, então desvio grande é reescalado em vez de recusado. O alinhamento no meio do vídeo continua sendo hipótese linear sem ground truth; os trechos seguem `approximate` e `ai_unreviewed`.
