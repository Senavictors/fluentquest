...
- **Credenciais**: `src/server/credentials.ts` — a chave de cada provedor vem de `credential()`, não mais de `process.env` direto. O proprietário cadastra em Ajustes (cifrada em `integration_credentials`, AES-256-GCM); a variável de ambiente é fallback. `AI_ENABLED`, `AI_TEXT_PROVIDER` e revisão de preço continuam só no ambiente — ADR-004.
